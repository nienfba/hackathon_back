...
var markerMouseOver = function(event)
{
    ...
    if ($selectFeature.length > 0) {
        ...
        $fl.stop().animate({scrollTop: '+=' + (selPos.top - fltop)+ 'px'});
    }
};
        ...
        initLayers() {
            this.layers.forEach((layer) => {
                ...
                markerFeatures.forEach((feature) => {
                    ...
                    var myIcon = L.divIcon({html: '<i class="fa-2x fas fa-' + icon + '"></i>', className: 'my-icon'});

                    feature.leafletObject = 
                        L.marker([feature.lat, feature.lon], {icon: myIcon}) //  feature.coords)
            ...
            updateMapEnd = function (event) {
                ...
                Vue.nextTick(function () {
                    ...
                    if ($selectFeature.length > 0) {
                        ...
                        var selPos = $selectFeature.position();
                        // animation relative sur la position scrolltop
                        $fl.animate({scrollTop: '+=' + (selPos.top - fltop)+ 'px'});
                    }
                });
